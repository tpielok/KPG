...
class SIVIAMortsteinLangevin(object):
    ...
    def learn(self):
        ...
        for epoch in tqdm(range(1, self.trainpara.num_epochs+1)):
            ...
            for i in range(1, self.trainpara.num_perepoch+1):
                ...
                #loss_kxy =  ((K.detach().matmul(compu_targetscore_aux.detach()) * X).sum(dim=1)).mean()
                
                
                #print(K.mean(1))
                #exit()
                #print((self.kernel(X.detach(),X_aux.detach()).matmul(neg_score_implicit_aux.detach()) * X).sum(dim=1).shape)
...
if __name__ == "__main__":
    ...
    task = SIVIAMortsteinLangevin(config)
    task.learn()
