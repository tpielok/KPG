...
class SIVIAMortstein2D(object):
    ...
    def learn(self):
        ...
        for j in range(3):
            ...
            for epoch in tqdm(range(1, self.trainpara.num_epochs+1), disable=not use_tqdm):
                with torch.no_grad():
                ...
                if (epoch - 1)%self.config.sampling.visual_time ==0 or epoch ==self.trainpara.num_epochs:
                    if self.target in ["banana", "multimodal", "x_shaped", "neal"]:
                        save_to_path = os.path.join(self.save_fig_to_path, str(self.iter_idx)+'.png')
                        bbox = {"multimodal":[-5, 5, -5, 5], "banana":[-3.5,3.5,-6,1], "x_shaped":[-5,5,-5,5], "neal": [-13, 13, -40, 40]}
                        quiver_plot = False
                ...
                for i in range(1, self.trainpara.num_perepoch+1):
                    ...
                    if not self.trainpara.ustat:
                        ...
                        K, h = self.kernel(X.detach(), X_aux, get_width=True)
                
                        
                        loss_kxy =   (((X_aux[:, None, :] - X.detach()[None, ...]).sum(dim=1)/h**2 - K.detach().matmul(compu_targetscore_aux.detach())) * X).sum(dim=1).mean()
                    ...
                    if i % (self.trainpara.num_perepoch//2) == 0: 
                        ...
                        logger.info("compu_targetscore: {:.4f}, neg_score_implicit: {:.4f}, f_norm, {:.4f}".format(compu_targetscore_aux.mean().item(), neg_score_implicit.abs().mean().item(),(compu_targetscore_aux + neg_score_implicit).mean().item()))
            
            torch.save(X.detach().cpu(),os.path.join(self.save_fig_to_path, str(self.iter_idx)+'.pt'))
            np.save(os.path.join(self.save_fig_to_path, str(j) + "loss"+'.npy'), np.array(loss_list))
            X = self.SemiVInet.sampling(num = self.config.sampling.num)
            torch.save(X.cpu().numpy(), os.path.join(self.config.log_path, str(j) + 'sample{}.pt'.format(self.config.sampling.num)))
            torch.save(self.SemiVInet, os.path.join(self.config.log_path,str(j) + "SemiVInet.ckpt"))
...
if __name__ == "__main__":
    ...
    config.log_path = os.path.join("amortexpkernelSIVI", config.target_score, "{}".format(datetimelabel))
