    ...
    def learn(self):
        ...
        for epoch in tqdm(range(1, self.trainpara.num_epochs+1)):
            ...
            if epoch% self.config.sampling.visual_time ==0:
                ...
                logger.info("compu_targetscore: {:.4f}, neg_score_implicit: {:.4f}, f_norm, {:.4f}, kernel_width: {:.4f}".format(compu_targetscore_aux.mean().item(), neg_score_implicit.abs().mean().item(),(compu_targetscore_aux + neg_score_implicit).mean().item(),kernel_width))
