            ...
            for epoch in tqdm(range(1, self.trainpara.num_epochs+1), disable=not use_tqdm):
                ...
                for i in range(1, self.trainpara.num_perepoch+1):
                    ...
                    if not self.trainpara.ustat:
                        ...
                        loss_kxy = (K.matmul(-neg_score_implicit_aux -  compu_targetscore_aux.detach()) * X).sum(dim=1).mean()
...
if __name__ == "__main__":
    ...
    config.log_path = os.path.join("pathexpkernelSIVI", config.target_score, "{}".format(datetimelabel))
